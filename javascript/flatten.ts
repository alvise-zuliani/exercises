...
function flattenArray(array: any[]): any[] {
    ...
    return array.reduce((acc: any, curr: any) => acc.concat(flatten(curr)), [])
}

function flattenObject(object: Record<string, any>): Record<string, any> {
    const flattenedObj: Record<string, any> = {};

    for (const [key, value] of Object.entries(object)) {
        // We iterate through the object's entries and call flatten.
        const valueIsObject: boolean = typeof value === 'object' && value !== null && !Array.isArray(value);
        const flattenedValue: any = flatten(value);
